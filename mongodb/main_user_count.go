...
func main() {
	...
	fromDate := time.Date(2020, time.May, 21, 0, 0, 0, 0, time.UTC)
	toDate := time.Date(2020, time.May, 22, 0, 0, 0, 0, time.UTC)
	...
	for i, j := range res {
		...
		// lineTags = fmt.Sprintf("%v", j["tags"])

		convertDate(j)

		// dateValid := j["month"].(int) == 3 || j["month"].(int) == 4
		// dateValid := true

		// if ok && lineTags != "[]" && lineUserID[0] == 'U' {
